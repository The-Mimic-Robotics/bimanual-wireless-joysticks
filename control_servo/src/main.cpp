...
void loop() {
  // Student Note: Serial.printf does not work or prints weird characters to terminal

  // Get potentiometers values from esp32 (2) to use for the offsets
  if (SerialBT.available())
  {
    // read potentiometers with: SerialBT.println(analogRead(POTENTIO_PIN));
    offsetServo1 = SerialBT.parseInt();
    offsetServo2 = SerialBT.parseInt();
    offsetServo3 = SerialBT.parseInt();
